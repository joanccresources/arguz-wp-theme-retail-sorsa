    ...
    if ($query->have_posts()):
      while ($query->have_posts()):
        ...
        if (!empty($atts['title'])) {
          ...
          $post_title_parts = explode(' ', $title, 2); // Dividir en dos partes
          if (count($post_title_parts) > 1) {
            $title = wp_kses_post('<span class="text-azul-sorsa">' . $atts['title']) . '</span> ' . $post_title_parts[1]; // Eliminar la primera parte y agregar el prefijo
          } else {
            $title = wp_kses_post('<span class="text-azul-sorsa">' . $atts['title']) . '</span> ' . $title; // Si el título solo tiene una parte
          }
        } else {
          // $title = esc_html($title); // Si no se proporciona un título, usar el título original
          ...
          $first_word = $words[0];
          $rest = $words[1] ?? '';
          $title = wp_kses_post('<span class="text-azul-sorsa">' . $first_word . '</span>' . ' ' . $rest); // Si el título solo tiene una parte
        }
